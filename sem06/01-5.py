...
def number(numb_a, numb_b):
    ...
    if (len(array_numb_a) == len(array_numb_b) == 2):
        # return round(math.sqrt(((array_numb_b[0] - array_numb_a[0])**2) + ((array_numb_b[1] - array_numb_a[1])**2)),2) - было
        return math.sqrt(sum(list(map(lambda x, y: math.pow((y - x),2), array_numb_a, array_numb_b)))) # стало
    else:
        return 'error'
